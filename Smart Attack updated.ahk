...
; 584,501 drop shield yes
; 678,250 put shield yes

;posX := [1048,624,1353,869,334,998,1865,1858,947,1059]
;posY := [731,164,952,664,386,753,397,397,302,349]

;posX := [1048,624,1353,334,998,1865,1858,1858,947,1059,1059]
;posY := [731,164,952,386,753,397,397,397,240,349,349] ;302

posX := [1048,624,1353,334,998,1865,1858,1858,947,1059]
posY := [731,164,952,386,753,397,397,397,302,349]
...
`::
    ...
    iterations := 1
		...
		if ( j = 1 ) ;click attack castle
		{
			sleep 800
               		Click, %x%, %y%
		}
		else if(j = 6) ;Speedup button center repeatedly
		{
			
			t1:=A_TickCount, X:=Y:=""
			Text:="|<>*170$18.zzzzzzzzzzzzzzzzzzzzzzzzzzzztzzlzzkzzUzzUzz4Tz6TzCDy0Dy07w07wTbwzXsznxznzzzzzzzzzzzzzzzzzzzzzU"
			while(ok:=FindText(681, 167, 797, 205, 0, 0, Text))
			{
				CoordMode, Mouse
		...
		else if ( j = 7) ;return march, click to remove dialogue box
		{
			Text:="|<>*147$72."
			if(ok:=FindText(843, 288, 1079, 340, 0, 0, Text))
			{
				CoordMode, Mouse
				sleep 100
   				Click, 948, 680 ;click ok to 'no army'
				sleep 300
				;Click %x%, %y% ;click away (at click top-left in case still in speedup screen)
			}
			else{
				sleep 400
				Click, %x%, %y%
...
Gui(cmd, arg1:="")
{
  ...
  if (!init)
  {
    ...
    Critical
    #NoEnv
    ww:=50, hh:=35, WindowColor:="0xDDEEFF"
